 ...
 * 
 * uses readdir() to get info about everything
 * 
 * bug: prints an empty string if run from "/";
 *      can not reach to "/" if in different filesystem.
 * 
 */
 ...
 */
{
    DIR * dir_ptr;                              /* 指向目录文件的指针 */
    struct dirent *direntp;                     /* 目录文件中的记录结构体指针 */
    dir_ptr = opendir(".");                     /* 返回目录指针 */
    if(dir_ptr == NULL){
        perror(".");
        exit(1);
    }
